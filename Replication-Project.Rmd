...
standard deviation is around 2.81. Meanwhile, the same variable in
multi-imputation model has a coefficient value of 0.093 with a 0.037 standard
error. That equates to around a 2.51 standard deviation, which is a pretty big
change from Bhandari's original results.
...
0.075 and a 0.032 standard error, which calculates to a 2.34 standard deviation.
However, with the multi-imputation data, this same variable has a coefficient of
0.066 and a 0.032 standard error, which calculates to a 2.06 standard deviation.
A 2.06 standard deviation borders on a variable not being significant in
conventional modeling terms (i.e., a >= 1.96 standard error is the cut-off for a
p value of < 0.05). These are all issues we must take into consideration when
